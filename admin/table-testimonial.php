...
$id=$_POST['id'];
  ...
  $dnk=$_FILES['image']['tmp_name'];
    $img="logo/".$image;
    move_uploaded_file($dnk,$img);
  $sql=mysqli_query($conn,"UPDATE `testimonial` SET `name`='$name',`position`='$position',`image`='$image',`rating`='$rating',`message`='$message' WHERE id='$id'");
...
if(isset($_GET['delid'])){
  ...
  if($sql=1){
      header("location:table-testimonial.php");
  }
  }
                        ...
                        ?>
                          ...
                          <div>
                            <button class="btn btn-default btn-xs dnkediti" data-id='<?php echo $arr['id']; ?>'><i class="fas fa-eye"></i></button> 
                                              ...
                                              <td class="p-2">
                                              <a href="table-testimonial.php?delid=<?php echo $arr['id']; ?>"><button
